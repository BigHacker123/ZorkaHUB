...
local Window = ZORKA.Utilities.UI:Window({
    Name = "ZORKA Hub — "..ZORKA.Game,
    ...
    local VisualsTab = Window:Tab({Name = "Visuals"}) do
        ...
        local LightingSection = VisualsTab:Section({Name = "Lighting",Side = "Right"}) do
            ...
            Callback = function(Bool) if Bool then return end
                for Property,Value in pairs(ZORKA.Utilities.Misc.DefaultLighting) do
    ...
    local SettingsTab = Window:Tab({Name = "Settings"}) do
        ...
        Callback = ZORKA.Utilities.Misc.ReJoin})
        SettingsTab:Button({Name = "Server Hop",Side = "Left",
        Callback = ZORKA.Utilities.Misc.ServerHop})
        SettingsTab:Button({Name = "Join Discord Server",Side = "Left",
        Callback = ZORKA.Utilities.Misc.JoinDiscord})
...
ZORKA.Utilities.Misc:SetupWatermark(Window)
ZORKA.Utilities.Misc:SetupLighting(Window.Flags)
ZORKA.Utilities.Drawing:SetupCursor(Window.Flags)

ZORKA.Utilities.Drawing:FOVCircle("Aimbot",Window.Flags)
ZORKA.Utilities.Drawing:FOVCircle("Trigger",Window.Flags)
ZORKA.Utilities.Drawing:FOVCircle("SilentAim",Window.Flags)
...
ZORKA.Utilities.Misc:NewThreadLoop(0,function()
...
ZORKA.Utilities.Misc:NewThreadLoop(1,function()
...
for Index,Item in pairs(Loot:GetDescendants()) do
    ...
    if Item:IsA("Model") and ItemData then --print(ItemData.Parent.Name)
        ZORKA.Utilities.Drawing:ItemESP(
...
Loot.DescendantRemoving:Connect(function(Item)
    if Item:IsA("Model") then
        ZORKA.Utilities.Drawing:RemoveESP(Item.Parent)
    end
end)
Randoms.ChildRemoved:Connect(function(Place)
    ZORKA.Utilities.Drawing:RemoveESP(Place)
end)
Vehicles.ChildRemoved:Connect(function(Vehicle)
    ZORKA.Utilities.Drawing:RemoveESP(Vehicle)
end)
Zombies.ChildRemoved:Connect(function(Zombie)
    ZORKA.Utilities.Drawing:RemoveESP(Zombie)
end)
...
for Index,Player in pairs(PlayerService:GetPlayers()) do
    ...
    ZORKA.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
end
