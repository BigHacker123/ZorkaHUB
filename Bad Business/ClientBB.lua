...
local Window = ZORKA.Utilities.UI:Window({
    ...
    local SettingsTab = Window:Tab({Name = "Settings"}) do
        ...
        local CreditsSection = SettingsTab:Section({Name = "Credits",Side = "Right"}) do
            CreditsSection:Label({Text = "This script was made by Zuhu"})
...
local GetFPS = ZORKA.Utilities.Misc:SetupFPS()
--repeat task.wait() until ZORKA.Utilities.Drawing.Cursor
ZORKA.Utilities.Drawing:Cursor(Window.Flags)
    ...
    for Index,Config in pairs(getgc(true)) do
        ...
        and rawget(Config,"Model") then
            if Config.Recoil and Config.Recoil.Default and
                ...
                local Modified = ZORKA.Config.GameFeatures.WeaponModification
...
Notify.Event:Connect(function(Text)
    ZORKA.Utilities.UI:Notification2(Text)
end)
    ...
    if GunModel then
        for Index,Instance in pairs(GunModel.Body:GetDescendants()) do
            ...
            and Instance.Reflectance < 1 then
                Instance.Color = ZORKA.Utilities.UI:TableToColor(Config.Color)
