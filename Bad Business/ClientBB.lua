...
Color = ZORKA.Utilities.Config:TableToColor(ZORKA.Config.UI.Color),Position = UDim2.new(0.2,-248,0.5,-248)}) do
    local AimAssistTab = Window:Tab({Name = "Combat"}) do
        ...
        local MiscSection = AimAssistTab:Section({Name = "Misc",Side = "Right"}) do
            ...
            MiscSection:Toggle({Name = "AutoShoot (Beta)",Value = ZORKA.Config.AimAssist.AutoShoot,Callback = function(Bool)
                ZORKA.Config.AimAssist.AutoShoot = Bool
            end}):ToolTip("Silent Aim will not work with this being toggled")
            MiscSection:Toggle({Name = "Trigger (Beta)",Value = ZORKA.Config.AimAssist.Trigger.Enabled,Callback = function(Bool)
                ZORKA.Config.AimAssist.Trigger.Enabled = Bool
            end})
            MiscSection:Slider({Name = "Delay",Min = 0,Max = 1,Precise = 2,Value = ZORKA.Config.AimAssist.Trigger.Delay,Callback = function(Number)
                ZORKA.Config.AimAssist.Trigger.Delay = Number
            end}):ToolTip("Trigger Delay")
            MiscSection:Slider({Name = "Hold Time",Min = 0,Max = 1,Precise = 2,Value = ZORKA.Config.AimAssist.Trigger.HoldTime,Callback = function(Number)
                ZORKA.Config.AimAssist.Trigger.HoldTime = Number
            end}):ToolTip("Trigger Hold Time")
            MiscSection:Dropdown({Name = "Priority",Default = ZORKA.Config.AimAssist.Trigger.Priority,List = {
                {Name = "Head",Mode = "Toggle",Callback = function(Selected)
                    ZORKA.Config.AimAssist.Trigger.Priority = Selected
                end},
                {Name = "Neck",Mode = "Toggle",Callback = function(Selected)
                    ZORKA.Config.AimAssist.Trigger.Priority = Selected
                end},
    ...
    local GameTab = Window:Tab({Name = ZORKA.Current}) do
        local GCSection = GameTab:Section({Name = "Gun Customization",Side = "Left"}) do
            GCSection:Toggle({Name = "Enabled",Value = ZORKA.Config.GameFeatures.GunCustomization.Enabled,Callback = function(Bool) 
                ZORKA.Config.GameFeatures.GunCustomization.Enabled = Bool
            end})
            GCSection:Toggle({Name = "Hide Textures",Value = ZORKA.Config.GameFeatures.GunCustomization.HideTextures,Callback = function(Bool) 
                ZORKA.Config.GameFeatures.GunCustomization.HideTextures = Bool
            end})
            GCSection:Colorpicker({Name = "Color",HSVAR = ZORKA.Config.GameFeatures.GunCustomization.Color,Callback = function(HSVAR)
                ZORKA.Config.GameFeatures.GunCustomization.Color = HSVAR
            end})
            GCSection:Slider({Name = "Reflectance",Min = 0,Max = 0.95,Precise = 2,Value = ZORKA.Config.GameFeatures.GunCustomization.Reflectance,Callback = function(Number)
                ZORKA.Config.GameFeatures.GunCustomization.Reflectance = Number
            end})
            GCSection:Dropdown({Name = "Material",Default = {ZORKA.Config.GameFeatures.GunCustomization.Material},List = {
                {Name = "SmoothPlastic",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.GunCustomization.Material = "SmoothPlastic"
                end},
                {Name = "ForceField",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.GunCustomization.Material = "ForceField"
                end},
                {Name = "Neon",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.GunCustomization.Material = "Neon"
                end},
                {Name = "Glass",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.GunCustomization.Material = "Glass"
                end}
            }})
        end
        local GMSection = GameTab:Section({Name = "Gun Modification",Side = "Left"}) do
            GMSection:Toggle({Name = "No Recoil",Value = ZORKA.Config.GameFeatures.GunModification.NoRecoil,Callback = function(Bool) 
                ZORKA.Config.GameFeatures.GunModification.NoRecoil = Bool
            end})
        end
        local EnvSection = GameTab:Section({Name = "Environment",Side = "Left"}) do
            EnvSection:Toggle({Name = "Enable",Value = ZORKA.Config.GameFeatures.Environment.Enabled,Callback = function(Bool)
                ZORKA.Config.GameFeatures.Environment.Enabled = Bool
                Lighting.ExposureCompensation = Bool and ZORKA.Config.GameFeatures.Environment.ExposureCompensation or 0
            end})
            EnvSection:Slider({Name = "Exposure Compensation",Min = -5,Max = 5,Precise = 2,Value = ZORKA.Config.GameFeatures.Environment.ExposureCompensation,Callback = function(Number)
                ZORKA.Config.GameFeatures.Environment.ExposureCompensation = Number
                if ZORKA.Config.GameFeatures.Environment.Enabled then
                    Lighting.ExposureCompensation = Number
                end
            end})
        end
        local ACSection = GameTab:Section({Name = "Arms Customization",Side = "Right"}) do
            ACSection:Toggle({Name = "Enabled",Value = ZORKA.Config.GameFeatures.ArmsCustomization.Enabled,Callback = function(Bool) 
                ZORKA.Config.GameFeatures.ArmsCustomization.Enabled = Bool
            end})
            ACSection:Toggle({Name = "Hide Textures",Value = ZORKA.Config.GameFeatures.ArmsCustomization.HideTextures,Callback = function(Bool) 
                ZORKA.Config.GameFeatures.ArmsCustomization.HideTextures = Bool
            end})
            ACSection:Colorpicker({Name = "Color",HSVAR = ZORKA.Config.GameFeatures.ArmsCustomization.Color,Callback = function(HSVAR)
                ZORKA.Config.GameFeatures.ArmsCustomization.Color = HSVAR
            end})
            ACSection:Slider({Name = "Reflectance",Min = 0,Max = 0.95,Precise = 2,Value = ZORKA.Config.GameFeatures.ArmsCustomization.Reflectance,Callback = function(Number)
                ZORKA.Config.GameFeatures.ArmsCustomization.Reflectance = Number
            end})
            ACSection:Dropdown({Name = "Material",Default = {ZORKA.Config.GameFeatures.ArmsCustomization.Material},List = {
                {Name = "SmoothPlastic",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.ArmsCustomization.Material = "SmoothPlastic"
                end},
                {Name = "ForceField",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.ArmsCustomization.Material = "ForceField"
                end},
                {Name = "Neon",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.ArmsCustomization.Material = "Neon"
                end},
                {Name = "Glass",Mode = "Button",Callback = function()
                    ZORKA.Config.GameFeatures.ArmsCustomization.Material = "Glass"
                end}
            }})
        end
        local CharSection = GameTab:Section({Name = "Character",Side = "Right"}) do
            CharSection:Toggle({Name = "Fly",Value = ZORKA.Config.GameFeatures.Character.Fly.Enabled,Callback = function(Bool) 
                ZORKA.Config.GameFeatures.Character.Fly.Enabled = Bool
            end}):Keybind({Key = ZORKA.Config.Binds.Fly,Callback = function(Bool,Key)
                ZORKA.Config.Binds.Fly = Key or "NONE"
            end})
            CharSection:Slider({Name = "Speed",Min = 10,Max = 100,Value = ZORKA.Config.GameFeatures.Character.Fly.Speed,Callback = function(Number)
                ZORKA.Config.GameFeatures.Character.Fly.Speed = Number
            end})
            CharSection:Toggle({Name = "Anti-Aim",Value = ZORKA.Config.GameFeatures.Character.AntiAim.Enabled,Callback = function(Bool) 
                ZORKA.Config.GameFeatures.Character.AntiAim.Enabled = Bool
            end}):Keybind({Key = ZORKA.Config.Binds.AntiAim,Callback = function(Bool,Key)
                ZORKA.Config.Binds.AntiAim = Key or "NONE"
            end})
            CharSection:Slider({Name = "Pitch",Min = -1.5,Max = 1.5,Precise = 2,Value = ZORKA.Config.GameFeatures.Character.AntiAim.Pitch,Callback = function(Number)
                ZORKA.Config.GameFeatures.Character.AntiAim.Pitch = Number
            end})
            CharSection:Slider({Name = "Pitch Random",Min = 0,Max = 1.5,Precise = 2,Value = ZORKA.Config.GameFeatures.Character.AntiAim.PitchRandom,Callback = function(Number)
                ZORKA.Config.GameFeatures.Character.AntiAim.PitchRandom = Number
            end})
        end
    end
...
for Index,Property in pairs({"ExposureCompensation"}) do
    if ZORKA.Config.GameFeatures.Environment.Enabled then
        Lighting[Property] = ZORKA.Config.GameFeatures.Environment[Property]
    end
    ...
    if GunModel then
        for Index,Instance in pairs(GunModel.Body:GetDescendants()) do
            ...
            and Instance.Reflectance < 1 then
                Instance.Color = ZORKA.Utilities.Config:TableToColor(Config.Color)
...
Toroiseshell.Network.Fire = function(self, ...)
    local args = {...}
    if ZORKA.Config.AimAssist.SilentAim.Enabled and SilentAim
    and not ZORKA.Config.AimAssist.AutoShoot then
        if args[2] == "__Hit" and math.random(0,100) <= ZORKA.Config.AimAssist.SilentAim.HitChance then
    ...
    if ZORKA.Config.GameFeatures.Character.AntiAim.Enabled and args[3] == "Look" then
        args[4] = ZORKA.Config.GameFeatures.Character.AntiAim.Pitch < -0
        and ZORKA.Config.GameFeatures.Character.AntiAim.Pitch + Random.new():NextNumber(0,
        ZORKA.Config.GameFeatures.Character.AntiAim.PitchRandom)
        or ZORKA.Config.GameFeatures.Character.AntiAim.Pitch - Random.new():NextNumber(0,
        ZORKA.Config.GameFeatures.Character.AntiAim.PitchRandom)
    end
...
Toroiseshell.Camera.Recoil.Fire = function(self,...)
    local args = {...}
    if ZORKA.Config.GameFeatures.GunModification.NoRecoil then
        args[1] = Vector2.zero
        args[2] = 0
        args[3] = 0
        args[4] = 0
...
RunService.Heartbeat:Connect(function()
    ...
    AutoShoot(ZORKA.Config.AimAssist.AutoShoot)
    PlayerFly(ZORKA.Config.GameFeatures.Character.Fly)
    CustomizeGun(ZORKA.Config.GameFeatures.GunCustomization)
    CustomizeArms(ZORKA.Config.GameFeatures.ArmsCustomization)
end)
ZORKA.Utilities.ThreadLoop(0,function()
    Trigger(ZORKA.Config.AimAssist.Trigger)
end)
